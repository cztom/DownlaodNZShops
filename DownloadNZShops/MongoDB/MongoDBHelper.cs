...
namespace DownloadNZShops
{
    ...
    public sealed class MongoDBHelper
    {
        ...
        {
            this._db = new MongoDBConfig().GetDataBase();
        }
        ...
        /// <param name="value">字段数值</param>
        /// <returns></returns>
        public T Find<T>(string key, string value)
        {
            string collectionName = typeof(T).Name;
            IMongoQuery query = Query.EQ(key, value);
            return FindOne<T>(query, collectionName);
        }
        ...
        /// </summary>
        /// <typeparam name="T">该数据所属的类型</typeparam>
        /// <param name="guid">ObjectId数值</param>
        /// <returns></returns>
        public T FindOneById<T>(string guid)
        {
            string collectionName = typeof(T).Name;
            return this._db.GetCollection<T>(collectionName).FindOneById(ObjectId.Parse(guid));
        }
        #endregion
